(() => {
    ...
    const LOCAL_STORAGE_KEY = "potato_carousel";
    ...
    async function fetchProducts() {
        try {
          ...
          console.error("Error occured details following:", err)
          return
        }
      }
                   ...
                   <button class="carousel-arrow left" data-carousel="${carouselId}" disabled>❮</button>
                    <div class="carousel" id="${carouselId}">
                        ${products.map((product, index) => {
                            return `<div class="carousel-item" data-index="${index}">
                                <div class="new-product-card">
                                    <div class="new-product-card__image-wrapper">
                                        ...
                                        <div class="new-product-card-like-button favorite ${isFavorite(product.id) ? 'favorited' : ''}" data-id="${product.id}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20.576" height="19.483" viewBox="0 0 20.576 19.483"><path fill="none" stroke="#555" stroke-width="1.5px" d="M19.032 7.111c-.278-3.063-2.446-5.285-5.159-5.285a5.128 5.128 0 0 0-4.394 2.532 4.942 4.942 0 0 0-4.288-2.532C2.478 1.826.31 4.048.032 7.111a5.449 5.449 0 0 0 .162 2.008 8.614 8.614 0 0 0 2.639 4.4l6.642 6.031 6.755-6.027a8.615 8.615 0 0 0 2.639-4.4 5.461 5.461 0 0 0 .163-2.012z" transform="translate(.756 -1.076)"/></svg>
                                        </div>
                                    </div>
    ...
    const setEvents = () => {
        ...
        $('.carousel-arrow').on('click', function(e) {
            ...
            const direction = $(this).hasClass('left') ? -1 : 1; //whether arrow direction is left or right
    ...
    const slideCarousel = (carouselId, direction) => {
        ...
        // Outer width includes margin.
        const itemWidth = $items.outerWidth(true);
        const wrapperWidth = $carousel.parent().width();
        const visibleItems = Math.floor(wrapperWidth / itemWidth); // Calculate,basically area per item(api :)), how many items can fit in the visible area
        ...
        $carousel.css('transform', `translateX(-${offset}px)`);//move carousel position to left or right
    ...
    const toggleFavorite = (id) => {
      ...
      //  console.log('toggleFavorite old =>', favorites);
        if (favorites.includes(id)) {
            favorites = favorites.filter(fav => fav !== id);
        } else {
            favorites.push(id);
        }
       // console.log('toggleFavorite new =>', favorites);
